...
def parse_json(text, name)
  ...
  fail_check("#{name} is invalid JSON: #{e.message}")
end

def load_required_texts
  required_files = {
    index_html: ["index.html"],
    app_js: ["js", "app.js"],
    feeds_json_text: ["data", "feeds.json"],
    feed_xml: ["data", "feed.xml"],
    checksums_json_text: ["data", "checksums.json"],
    readme_text: ["README.md"],
    cname_text: ["CNAME"],
    headers_text: ["_headers"],
    fetch_script: ["scripts", "fetch_feeds.py"],
    site_config_text: ["config", "site.json"]
  }

  required_files.each_with_object({}) do |(key, parts), texts|
    texts[key] = read_text(ROOT.join(*parts))
  end
end

def validate_canonical_config!(texts)
  site_config = parse_json(texts[:site_config_text], "config/site.json")
  canonical_host = site_config["canonicalHost"]
  canonical_url = site_config["canonicalUrl"]
  configured_cname = texts[:cname_text].strip
  csp_match = texts[:headers_text].match(/Content-Security-Policy:\s*(.+)$/)
  ...
  assert(texts[:index_html].include?("content=\"#{canonical_url}\""), "index.html og:url must match config/site.json canonicalUrl")
  assert(texts[:index_html].include?("content=\"#{canonical_url}/og-image.png\""), "index.html og:image must match config/site.json canonicalUrl")
  assert(texts[:readme_text].include?("[#{canonical_host}](#{canonical_url})"), "README live-site link must match config/site.json canonical URL")
  assert(texts[:readme_text].include?("`#{canonical_url}`"), "README setup URL must match config/site.json canonical URL")
  assert(!texts[:readme_text].include?("cloudplatformfeed.kailice.uk"), "README must not reference legacy domain")
  assert(texts[:feed_xml].include?("<link>#{canonical_url}</link>"), "data/feed.xml channel link must match config/site.json canonicalUrl")
  assert(texts[:fetch_script].include?("load_site_config"), "fetch_feeds.py must load canonical URL from config/site.json")
  assert(!texts[:fetch_script].include?(canonical_host), "fetch_feeds.py should not hardcode canonical host")
end

def validate_frontend_assets!(texts)
  [texts[:index_html], texts[:app_js]].each do |content|
  ...
  expected_scripts = [
    ...
    "js/modules/checksum-watcher.js",
    "js/modules/filter-helpers.js"
  ]
  expected_scripts.each do |script_path|
    escaped_path = Regexp.escape(script_path)
    assert(
      texts[:index_html].match?(%r{<script\s+src="#{escaped_path}(?:\?[^"]+)?"\s+defer></script>}),
      "index.html does not load #{script_path} with defer"
    )
  end

  assert(!texts[:index_html].include?("js/clarity.js"), "index.html should not reference removed js/clarity.js")
  assert(texts[:index_html].include?("Content-Security-Policy"), "index.html is missing a Content-Security-Policy")
  assert(texts[:index_html].include?("style-src 'self'"), "index.html CSP should restrict styles to self")
  assert(!texts[:index_html].include?("style-src 'self' 'unsafe-inline'"), "index.html CSP should not allow unsafe-inline styles")
  assert(!texts[:index_html].include?("script-src 'self' 'unsafe-inline'"), "index.html CSP should not allow unsafe-inline scripts")
  assert(!texts[:index_html].include?("www.clarity.ms/tag/\"+i"), "index.html should not inline the Clarity bootstrap")
  assert(texts[:index_html].include?("id=\"articles-grid\""), "index.html is missing articles grid container")
  assert(texts[:index_html].include?("id=\"filter-pills\""), "index.html is missing filter pills container")
  assert(texts[:index_html].include?("id=\"category-selection-summary\""), "index.html is missing category selection summary")
  assert(texts[:index_html].include?("id=\"clear-category-selection\""), "index.html is missing clear category selection button")
  assert(texts[:app_js].include?("Ctrl/Cmd+click to multi-select categories"), "js/app.js is missing multi-select category affordance")
  assert(texts[:app_js].include?("updateCategorySelectionSummary"), "js/app.js is missing category selection summary logic")
end

def validate_security_headers!(headers_text)
...
def validate_summary_metadata!(feeds)
...
def validate_techcommunity_articles!(articles)
...
def run_smoke_checks!
  texts = load_required_texts
  validate_canonical_config!(texts)
  validate_frontend_assets!(texts)
  validate_security_headers!(texts[:headers_text])

  feeds = parse_json(texts[:feeds_json_text], "data/feeds.json")
  checksums = parse_json(texts[:checksums_json_text], "data/checksums.json")
  ...
  assert(texts[:feed_xml].include?("<rss") || texts[:feed_xml].include?("<feed"), "feed.xml does not look like RSS or Atom")

  validate_checksums!(checksums)
  validate_summary_metadata!(feeds)
  validate_articles!(articles)
  validate_techcommunity_articles!(articles)
end
